    ...
    with st.container():
        ...
        with col1:
            if "Estado" in df.columns:
                filters["Estado"] = st.selectbox("Estado", ["Todos"] + sorted(df["Estado"].dropna().unique()))
        with col2:
            if "Sector" in df.columns:
                filters["Sector"] = st.selectbox("Sector", ["Todos"] + sorted(df["Sector"].dropna().unique()))
        with col3:
            if "Tipo de reclamo" in df.columns:
                filters["Tipo de reclamo"] = st.selectbox("Tipo de reclamo", ["Todos"] + sorted(df["Tipo de reclamo"].dropna().unique()))
